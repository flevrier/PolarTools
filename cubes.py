...
class Map(PhysicalQuantity):
    """Map : a 2D array of physical quantities, two axes, and a type (scalar or vector)"""
    def __init__(self,
...
class Cube(PhysicalQuantity):
    """Map : a 3D array of physical quantities, three axes, and a type (scalar or vector)"""
    def __init__(self,
    ...
    def readfromfits(self,fitsfile):
        ...
        # Read data unit
        if (h['BUNIT']=="cm^-3"):self.unit=u.cm**-3
        if (h['BUNIT']=="Gauss"):self.unit=u.G
        self.v=d*self.unit
        if (h['CUNIT1']=="pc"):unit1=u.pc
        if (h['CUNIT2']=="pc"):unit2=u.pc
        if (h['CUNIT3']=="pc"):unit3=u.pc
        self.axis1=Axis(value=float(h['CRVAL1'])+float(h['CDELT1'])*(np.linspace(0.0,float(int(h['NAXIS1']))-1.0,num=int(h['NAXIS1']))-float(h['CRPIX1'])),unit=unit1,descr=h['CTYPE1'])
        self.axis2=Axis(value=float(h['CRVAL2'])+float(h['CDELT2'])*(np.linspace(0.0,float(int(h['NAXIS2']))-1.0,num=int(h['NAXIS2']))-float(h['CRPIX2'])),unit=unit2,descr=h['CTYPE2'])
        self.axis3=Axis(value=float(h['CRVAL3'])+float(h['CDELT3'])*(np.linspace(0.0,float(int(h['NAXIS3']))-1.0,num=int(h['NAXIS3']))-float(h['CRPIX3'])),unit=unit3,descr=h['CTYPE3'])
    ...
    def PDF(self,scaling="lin",nbins=200,normed=0,density=True):
        ...
        if(xscal=="lin"):hist,bin_edges=np.histogram(v_1D, nbins, normed=normed,density=density)
        if(xscal=="log"):hist,bin_edges=np.histogram(np.log10(v_1D), nbins, normed=normed,density=density)        
